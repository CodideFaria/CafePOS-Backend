...
def test_inventory():
    ...
    menu_item_data = {"name": "Latte", "size": "Medium", "price": 3.50}
    ...
    created_inventory_id = created_inventory_item['id']
    ...
    assert len(response.json()['inventory_items']) > 0
    ...
    print(f"Response: {response.json()}")
    assert response.status_code == 200
    assert response.json()['id'] == created_inventory_id

    # 4. Update the created inventory item
    print(f"\nUpdating inventory item with ID: {created_inventory_id}...")
    update_data = {"quantity": 90}
